...
When a fix compensates at the point of use (an offset, a `+1`/`-1`, a filter, a clamp, a `< 2` special case), the error is usually in the definition of the data being consumed. Fix it there, once, so every consumer gets it right and no call site carries special knowledge. Tells: the same compensation repeated at several sites; a sibling that already does it right by construction. Applies to reviewing PRs as much as to writing code — accept the diagnosis, redirect the fix.
